...
- `macrofactor` (`src/macrofactor.ts`): export recent foods plus nutrition from MacroFactor's Firebase/Firestore API using `MACROFACTOR_CREDENTIALS=<email>:<password>`. Keeps the grouped recent-food output and supports `--days`, `--start`, `--end`, `--limit`, `--format=json|table|csv`, `--output`, and `--pretty`.
