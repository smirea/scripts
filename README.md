...
- `era-fit` (`src/era-fit.ts`): checks whether a stored `ERA_FIT_SESSION_COOKIE` can load the Era Fit client dashboard, or logs in with prompted credentials / `ERA_FIT_CREDENTIALS=<email>:<password>` and saves the resulting session cookie to `.env.local`. Use `--no-login` to test whether the stored cookie alone is enough.
