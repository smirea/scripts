...
- `convex` (`src/convex.ts`): manage local self-hosted Convex deployments. `convex create -n <name>` creates an isolated Docker Compose deployment under `~/code/convex-deployments/<name>` by default, prints the app env vars, and registers macOS boot start unless `--no-boot` is passed. `convex restart <id>`, `convex dashboard <id>`, and `convex delete <id>` accept either a standard deployment folder name or a full deployment path.
