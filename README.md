...
- `era-fit` (`src/era-fit/index.ts`): exports Era Fit daily nutrition targets and logged foods from the API using a stored `ERA_FIT_SESSION_COOKIE`, or logs in with prompted credentials / `ERA_FIT_CREDENTIALS=<email>:<password>` and saves the resulting session cookie to `.env.local`. The default command and `era-fit log` print the daily macro log with `--date`, `--days`, `--start`, `--end`, `--limit`, `--format=json|table|csv|csv:full`, and `--output`. `era-fit mealplan` prints the weekly suggested meal plan with `--today`, `--anylist`, `--format=json|table`, and `--output`.
