...
- `macrofactor` (`src/macrofactor.ts`): export recent foods plus nutrition from MacroFactor's local `historyFood.json` cache (defaults to the last 7 days). Supports `--format=json|table|csv` (default `table`).
