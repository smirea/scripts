...
- `macrofactor` (`src/macrofactor.ts`): export recent foods plus nutrition from MacroFactor's Firebase/Firestore API using `MACROFACTOR_CREDENTIALS=<email>:<password>`, or `MACROFACTOR_FIREBASE_REFRESH_TOKEN` when Firebase App Check blocks password sign-in. Successful auth saves or rotates `MACROFACTOR_FIREBASE_REFRESH_TOKEN` in `.env.local` automatically. Uses standardized `serving` / `servingGrams` fields across all outputs, includes daily scale weight and body fat when present, and JSON normalizes recipes to the same item shape with `kind` plus `recipeId` when applicable. Supports `--days`, `--start`, `--end`, `--limit`, `--format=json|table|csv|csv:full`, `--output`, `--pretty`, and `--full` (`--full` expands the detailed-food CSV/table nutrient columns, including inside `csv:full`).
