...
- `era-fit` (`src/era-fit/index.ts`): exports Era Fit daily nutrition targets and logged foods from the API using a cached `ERA_FIT_SESSION_COOKIE` when it still works, otherwise refreshing it from `ERA_FIT_CREDENTIALS=<email>:<password>`. If the cookie fails and credentials are missing or rejected in an interactive shell, it prompts for them and saves the working value to `.env.local`. `era-fit` with no subcommand prints help. `era-fit print-food` prints the daily macro log with `--date`, `--days`, `--start`, `--end`, `--limit`, `--format=json|table|csv|csv:full`, and `--output`. `era-fit track` logs foods with items separated by `,`, for example `era-fit track b 50g chicken breast, 1 banana`. `era-fit mealplan` prints the weekly suggested meal plan with `--today`, `--anylist`, `--format=json|table`, and `--output`.
- `macrofactor` (`src/macrofactor.ts`): export MacroFactor nutrition from the local macOS app Firestore cache. It opens MacroFactor at most once per day to let the app sync first; pass `--no-cache` to force that refresh. The cache currently exposes daily nutrition totals, program targets, scale weight, and body fat; detailed per-food rows and recipe breakdowns are emitted when `food` / `customFoods` documents are present in the local cache. Supports `--days`, `--start`, `--end`, `--limit`, `--format=json|table|csv|csv:full`, `--output`, `--pretty`, and `--full` (`--full` expands the detailed-food CSV/table nutrient columns, including inside `csv:full`).
